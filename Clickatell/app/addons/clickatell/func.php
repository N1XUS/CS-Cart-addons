...
function fn_clickatell_get_shipping_methods() {
...
function fn_clickatell_get_order_statuses() {
...
function fn_clickatell_create_shipment(&$shipment_data, &$order_info, &$group_key, &$all_products) {
    ...
    if ($notify_user == true) {
        ...
        $path = Registry::get('config.dir.addons') . 'clickatell/';
        
        include ($path . 'vendor/autoload.php');
        
        $phone_area = Registry::get('addons.clickatell.customer_phone_field');
        ...
        if (empty($phone)) {
            // If this field is empty, then try to use default phone field
        ...
        $body = Tygh::$app['view']->fetch('addons/clickatell/views/sms/components/new_shipment_sms.tpl');
        
        $body = fn_clickatell_strip_tags($body);
        
        fn_clickatell_send_sms($phone, $body);
    }
}

function fn_clickatell_change_order_status(&$status_to, &$status_from, &$order_info, &$force_notification, &$order_statuses, &$place_order) {

    if (Registry::get('addons.clickatell.admin_sms_order_updated') == 'Y' || Registry::get('addons.clickatell.customer_sms_order_updated') == 'Y') {
        ...
        if (Registry::get('addons.clickatell.admin_sms_order_updated') == 'Y' && $place_order == false) {
            $result = fn_clickatell_check_order_conditions('admin', $status_to, $order_info, $order_statuses);
            if ($result == true) {
                $body = Tygh::$app['view']->fetch('addons/clickatell/views/sms/components/admin_order_updated_sms.tpl');
                $body = fn_clickatell_strip_tags($body);
                $phone = Registry::get('addons.clickatell.admin_phone_number');
                $phone = explode(',', $phone);
                foreach($phone as $k => $v) {
                    fn_clickatell_send_sms($v, $body);
                }
            }
        }
        
        if (Registry::get('addons.clickatell.customer_sms_order_updated') == 'Y') {
            ...
            $result = fn_clickatell_check_order_conditions('customer', $status_to, $order_info, $order_statuses);
...
function fn_clickatell_place_order(&$order_id, &$action, &$fake1, &$cart) {
    if ($action !== 'save' && Registry::get('addons.clickatell.admin_sms_new_order_placed') == 'Y') {
        Tygh::$app['view']->assign('order_id', $order_id);
        Tygh::$app['view']->assign('total', $cart['total']);
        
        $send_info = Registry::get('addons.clickatell.admin_sms_send_payment_info');
        $send_email = Registry::get('addons.clickatell.admin_sms_send_customer_email');
        $send_min_amount = Registry::get('addons.clickatell.admin_sms_send_min_amout');
        $shippings = Registry::get('addons.clickatell.admin_sms_send_shipping');
        ...
        if ($in_shipping && $order['subtotal'] > doubleval($send_min_amount)) {
            $body = Tygh::$app['view']->fetch('addons/clickatell/views/sms/components/admin_order_place_sms.tpl');
            $body = fn_clickatell_strip_tags($body);
            $phone = Registry::get('addons.clickatell.admin_phone_number');
            $phone = explode(',', $phone);
            foreach($phone as $k => $v) {
...
function fn_clickatell_update_profile(&$action, &$user_data) {
    if ($action == 'add' && AREA == 'C' && Registry::get('addons.clickatell.admin_sms_new_cusomer_registered') == 'Y') {
        Tygh::$app['view']->assign('customer', $user_data['firstname'] . (empty($user_data['lastname']) ? '' : $user_data['lastname']));
        $body = Tygh::$app['view']->fetch('addons/clickatell/views/sms/components/new_profile_sms.tpl');
        $body = fn_clickatell_strip_tags($body);
        $phone = Registry::get('addons.clickatell.admin_phone_number');
        $phone = explode(',', $phone);
        foreach($phone as $k => $v) {
...
function fn_clickatell_update_product_amount(&$new_amount, &$product_id) {
    if ($new_amount <= Registry::get('settings.General.low_stock_threshold') && Registry::get('addons.clickatell.admin_sms_product_negative_amount') == 'Y') {
        ...
        $body = Tygh::$app['view']->fetch('addons/clickatell/views/sms/components/low_stock_sms.tpl');
        $body = fn_clickatell_strip_tags($body);
        $phone = Registry::get('addons.clickatell.admin_phone_number');
        $phone = explode(',', $phone);
        foreach($phone as $k => $v) {
...
function fn_clickatell_check_order_conditions($for = 'admin', $status_to, $order, $order_statuses) {
    $send_min_amount = Registry::get('addons.clickatell.' . $for . '_sms_send_min_amount');
    $shippings = Registry::get('addons.clickatell.' . $for . '_sms_send_shipping');
    $statuses = Registry::get('addons.clickatell.' . $for . '_sms_send_order_statuses');
    if ($for == 'admin') {
        $send_email = Registry::get('addons.clickatell.admin_sms_send_customer_email');
    }
...
function fn_clickatell_send_sms($phone, $body) {
    
    $access_data = fn_clickatell_get_sms_auth_data();
    if (fn_is_empty($access_data) || empty($phone) || !$body) {
        return false;
    }

    $concat = Registry::get('addons.clickatell.clickatell_concat');
    //get the last symbol
    if (!empty($concat)) {
    ...
    $unicode = Registry::get('addons.clickatell.clickatell_unicode') == 'Y' ? 1 : 0;
...
function fn_clickatell_get_sms_auth_data()
{
     return array('login' => Registry::get('addons.clickatell.clickatell_user'),
                  'password' => Registry::get('addons.clickatell.clickatell_password') ,
                  'api_id' => Registry::get('addons.clickatell.clickatell_api_id'));
}
 ...
 */
function fn_clickatell_strip_tags(&$var)
{
