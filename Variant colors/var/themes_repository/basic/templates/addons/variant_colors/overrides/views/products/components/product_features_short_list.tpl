...
{function name="feature_value"}
    {strip}
        ...
        {elseif $feature.feature_type == "ProductFeatures::TEXT_SELECTBOX"|enum || $feature.feature_type == "ProductFeatures::NUMBER_SELECTBOX"|enum || $feature.feature_type == "ProductFeatures::EXTENDED"|enum}
            {if $feature.color == "Y" && $feature["variants"][$feature.variant_id]["color"]}<span style="background: {$feature["variants"][$feature.variant_id]["color"]}" class="ty-feature-color__icon"></span>{/if}{$feature.variant|default:$feature.value}
        ...
        {else}
            {if $feature.color == "Y" && $feature["variants"][$feature.variant_id]["color"]}<span style="background: {$feature["variants"][$feature.variant_id]["color"]}" class="ty-feature-color__icon"></span>{/if}{$feature.value}
        {/if}
