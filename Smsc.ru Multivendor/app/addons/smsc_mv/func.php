...
function fn_smsc_mv_get_shipping_methods() {
...
function fn_smsc_mv_get_order_statuses() {
...
function fn_smsc_mv_create_shipment(&$shipment_data, &$order_info, &$group_key, &$all_products) {
    ...
    if ($notify_user == true) {
        ...
        $path = Registry::get('config.dir.addons') . 'smsc_mv/';
        
        include ($path . 'vendor/autoload.php');
        
        $phone_area = Registry::get('addons.smsc_mv.customer_phone_field');
        ...
        $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/new_shipment_sms.tpl');
        
        $body = fn_smsc_mv_strip_tags($body);
        
        fn_smsc_mv_send_sms($phone, $body);
    }
}

function fn_smsc_mv_change_order_status(&$status_to, &$status_from, &$order_info, &$force_notification, &$order_statuses, &$place_order) {

    if (Registry::get('addons.smsc_mv.admin_sms_order_updated') == 'Y' || Registry::get('addons.smsc_mv.customer_sms_order_updated') == 'Y' || Registry::get('addons.smsc_mv.vendor_sms_order_updated') == 'Y') {
        ...
        if (Registry::get('addons.smsc_mv.admin_sms_order_updated') == 'Y' && $place_order == false) {
            $result = fn_smsc_mv_check_order_conditions('admin', $status_to, $order_info, $order_statuses);
            if ($result == true) {
                $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/admin_order_updated_sms.tpl');
                $body = fn_smsc_mv_strip_tags($body);
                $phone = Registry::get('addons.smsc_mv.admin_phone_number');
                $phone = explode(',', $phone);
                foreach($phone as $k => $v) {
                    fn_smsc_mv_send_sms($v, $body);
                }
            }
        }
        
        if (Registry::get('addons.smsc_mv.vendor_sms_order_updated') == 'Y' && $place_order == false) {
            $result = fn_smsc_mv_check_order_conditions('vendor', $status_to, $order_info, $order_statuses);
            if ($result == true) {
                $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/admin_order_updated_sms.tpl');
                $body = fn_smsc_mv_strip_tags($body);
                $phone = fn_smsc_mv_get_company_phone($order_info['company_id']);
                $phone = explode(',', $phone);
                foreach($phone as $k => $v) {
                    fn_smsc_mv_send_sms($v, $body);
                }
        ...
        if (Registry::get('addons.smsc_mv.customer_sms_order_updated') == 'Y') {
            ...
            $result = fn_smsc_mv_check_order_conditions('customer', $status_to, $order_info, $order_statuses);
            if ($result == true) {
    ...
    if ($place_order == true && Registry::get('addons.smsc_mv.vendor_sms_new_order_placed') == 'Y') {
        $order_id = $order_info['order_id'];
        ...
        $result = fn_smsc_mv_check_order_conditions('vendor', '', $order_info, array(), true);
        
        if ($result == true) {
            $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/admin_order_place_sms.tpl');
            $body = fn_smsc_mv_strip_tags($body);
            $phone = fn_smsc_mv_get_company_phone($order_info['company_id']);
            $phone = explode(',', $phone);
            foreach($phone as $k => $v) {
...
function fn_smsc_mv_place_order(&$order_id, &$action, &$fake1, &$cart) {
    if ($action !== 'save' && Registry::get('addons.smsc_mv.admin_sms_new_order_placed') == 'Y') {
        ...
        $result = fn_smsc_mv_check_order_conditions('admin', '', $order, array(), true);
        if ($result == true) {
            $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/admin_order_place_sms.tpl');
            $body = fn_smsc_mv_strip_tags($body);
            $phone = Registry::get('addons.smsc_mv.admin_phone_number');
...
function fn_smsc_mv_update_profile(&$action, &$user_data) {
    if ($action == 'add' && AREA == 'C' && (Registry::get('addons.smsc_mv.admin_sms_new_cusomer_registered') == 'Y' || Registry::get('addons.smsc_mv.vendor_sms_new_cusomer_registered') == 'Y')) {
        Tygh::$app['view']->assign('customer', $user_data['firstname'] . (empty($user_data['lastname']) ? '' : $user_data['lastname']));
        $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/new_profile_sms.tpl');
        $body = fn_smsc_mv_strip_tags($body);
        if (Registry::get('addons.smsc_mv.admin_sms_new_cusomer_registered') == 'Y') {
            $phone = Registry::get('addons.smsc_mv.admin_phone_number');
            $phone = explode(',', $phone);
            foreach($phone as $k => $v) {
                fn_smsc_mv_send_sms($v, $body);
...
function fn_smsc_mv_update_product_amount(&$new_amount, &$product_id) {
    if ($new_amount <= Registry::get('settings.General.low_stock_threshold') && (Registry::get('addons.smsc_mv.admin_sms_product_negative_amount') == 'Y' || Registry::get('addons.smsc_mv.vendor_sms_product_negative_amount') == 'Y')) {
        ...
        $body = Tygh::$app['view']->fetch('addons/smsc_mv/views/sms/components/low_stock_sms.tpl');
        $body = fn_smsc_mv_strip_tags($body);
        if (Registry::get('addons.smsc_mv.admin_sms_product_negative_amount') == 'Y') {
            $phone = Registry::get('addons.smsc_mv.admin_phone_number');
            $phone = explode(',', $phone);
            foreach($phone as $k => $v) {
                fn_smsc_mv_send_sms($v, $body);
        ...
        if (Registry::get('addons.smsc_mv.vendor_sms_product_negative_amount') == 'Y') {
            $company_id = db_get_field("SELECT company_id FROM ?:products WHERE product_id = ?i", $product_id);
            $phone = fn_smsc_mv_get_company_phone($company_id);
            $phone = explode(',', $phone);
            foreach($phone as $k => $v) {
                fn_smsc_mv_send_sms($v, $body);
            }            
...
function fn_smsc_mv_check_order_conditions($for = 'admin', $status_to = '', $order, $order_statuses = array(), $skip_statuses = false) {
    $send_min_amount = Registry::get('addons.smsc_mv.' . $for . '_sms_send_min_amount');
    $shippings = Registry::get('addons.smsc_mv.' . $for . '_sms_send_shipping');
    if ($skip_statuses == false) {
        $statuses = Registry::get('addons.smsc_mv.' . $for . '_sms_send_order_statuses');
    } else {
        $statuses = array();
    }
...
function fn_smsc_mv_get_company_phone($company_id) {
    $company_data = fn_get_company_data($company_id);
    $phone = $company_data['phone'];
    return $phone;
}

function fn_smsc_mv_send_sms($phone, $body) {
    ...
    $balance = $client->get_balance(fn_smsc_mv_get_auth());
    ...
    if ($balance > 0) {
        ...
        $sms = array(
            'login' => Registry::get('addons.smsc_mv.smsc_login'),
            'psw' => Registry::get('addons.smsc_mv.smsc_password'),
            'sender' => Registry::get('addons.smsc_mv.smsc_sender'),
            'phones' => $phone,
            'mes' => $text
        );
 ...
 */
function fn_smsc_mv_strip_tags(&$var)
{
